...
public class TerminalFragment extends Fragment implements ServiceConnection, SerialListener {
    ...
    private MaskEditText editText2;
    ...
    private TextView sendText2; //MasiHND
    private ImageButton sendBtn;
    private TextUtil.HexWatcher hexWatcher;
    private TextUtil.HexWatcher hexWatcher2; //MasiHND
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        sendText2 = view.findViewById(R.id.send_text2);     //MasiHND

        //editText = view.findViewById(R.id.send_text);        //MasiHND
        //editText2 = view.findViewById(R.id.send_text2);      //MasiHND

        sendBtn = view.findViewById(R.id.send_btn);
        hexWatcher = new TextUtil.HexWatcher(sendText);
        hexWatcher2 = new TextUtil.HexWatcher(sendText2);    //MasiHND
        hexWatcher.enable(hexEnabled);
        hexWatcher2.enable(hexEnabled);                      //MasiHND
        sendText.addTextChangedListener(hexWatcher);
        //sendText.setHint(hexEnabled ? "HEX mode" : "");
        sendText2.addTextChangedListener(hexWatcher2);       //MasiHND
        //sendText2.setHint(hexEnabled ? "HEX mode" : "");   //MasiHND

        View sendBtn = view.findViewById(R.id.send_btn);
        //sendBtn.setOnClickListener(v -> send(sendText.getText().toString()));
        //controlLines.onCreateView(view);
        sendBtn.setOnClickListener(v -> send(sendText.getText().toString() + sendText2.getText().toString())); //MasiHND
        controlLines.onCreateView(view);    //MasiHND
        return view;
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.clear) {
            ...
            sendText.setText("");
            hexWatcher.enable(hexEnabled);
            //sendText.setHint(hexEnabled ? "HEX mode" : "");    //MasiHND
            sendText2.setText("");
            hexWatcher2.enable(hexEnabled);
            //sendText2.setHint(hexEnabled ? "HEX mode" : "");
            ...
            try {
                ...
                error("send BREAK failed: " + e.getMessage());
            }
            return true;
        }
    ...
    private void connect(Boolean permissionGranted) {
        ...
        if(driver == null) {
            error("connection failed: no driver for device");
            return;
        }
        if(driver.getPorts().size() < portNum) {
            error("connection failed: not enough ports at device");
            return;
        }
        ...
        if(usbConnection == null) {
            if (!usbManager.hasPermission(driver.getDevice()))
                ...
                error("connection failed: open failed");
            return;
        }
        ...
        try {
            ...
            try {
                ...
                error("Setting serial parameters failed: " + e.getMessage());
            }
    ...
    private void send(String str) {      //MasiHND
        if(connected != Connected.True) {
    ...
    void error(String str) {
        SpannableStringBuilder spn = new SpannableStringBuilder(str + '\n');
        spn.setSpan(new ForegroundColorSpan(getResources().getColor(R.color.colorErrorText)), 0, spn.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        receiveText.append(spn);
    }

    void warning(String str) {
        SpannableStringBuilder spn = new SpannableStringBuilder(str + '\n');
        spn.setSpan(new ForegroundColorSpan(getResources().getColor(R.color.colorWarningText)), 0, spn.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        receiveText.append(spn);
    }
    ...
    class ControlLines {
        ...
        void selectFlowControl() {
            ...
            builder.setSingleChoiceItems(names.toArray(new CharSequence[0]), pos, (dialog, which) -> {
                ...
                try {
                    ...
                    error("Set flow control failed: "+e.getClass().getName()+" "+e.getMessage());
        ...
        void start() {
            if (showControlLines) {
                try {
                    ...
                    error("getSupportedControlLines() failed: " + e.getMessage());
                }
            }
        ...
        private void run() {
            ...
            try {
                ...
                error("getControlLines() failed: " + e.getMessage() + " -> stopped control line refresh");
            }
        }
        ...
        private void toggle(View v) {
            ...
            try {
                ...
                error("set" + ctrl + " failed: " + e.getMessage());
            }
        }
